...
from bayesian_benchmarks import paths as bbp
...
class Dataset(object):
    def __init__(self, split=0, prop=0.9):
        ...
        np.random.seed(bbp.BASE_SEED + split)
    ...
    @property
    def datadir(self):
        dir = os.path.join(bbp.DATA_PATH, self.name)
...
class Classification(Dataset):
    ...
    def needs_download(self):
        if os.path.isfile(
            os.path.join(bbp.DATA_PATH, "classification_data", "iris", "iris_R.dat")
        ):
            return False
        else:
            return True
    ...
    def download(self):
        ...
        filename = os.path.join(bbp.DATA_PATH, "classification_data.tar.gz")
        ...
        tar.extractall(path=os.path.join(bbp.DATA_PATH, "classification_data"))
        tar.close()
    ...
    def read_data(self):
        datapath = os.path.join(
            bbp.DATA_PATH, "classification_data", self.name, self.name + "_R.dat"
        )
...
class NYTaxiTimePrediction(NYTaxiBase):
    ...
    def read_data(self):
        path = os.path.join(bbp.DATA_PATH, "taxitime_preprocessed.npz")
...
class NYTaxiLocationPrediction(NYTaxiBase):
    ...
    def read_data(self):
        path = os.path.join(bbp.DATA_PATH, "taxiloc_preprocessed.npz")
        if os.path.isfile(path):
            with open(path, "rb") as file:
                f = np.load(file)
                X, Y = f["X"], f["Y"]
...
class WilsonDataset(Dataset):
    ...
    def datapath(self):
        ...
        return "{}/uci/{}/{}.mat".format(bbp.DATA_PATH, n, n)
